...
// Make a raw api call
func (a API) Raw(client *http.Client, u interface{}, out interface{}) (e error) {
	endpoint := url.URL{} // initialize the url

	switch t := u.(type) {
	default:
		return errors.New(fmt.Sprintf("Expecting string or *url.URL, got %v: %#v", t, u))
	...
	err = json.Unmarshal(data, out) // convert the response data to json

	if err != nil {
		var d map[string]interface{}
		newErr := json.Unmarshal(data, &d)
		if newErr != nil {
			return newErr
		}
